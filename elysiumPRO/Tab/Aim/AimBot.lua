...
function AimBot.createAimbotCircle(screenGui)
...
function AimBot.updateAimbotCircle()
    if aimbotCircle then
        aimbotCircle.Size = UDim2.new(0, aimbotFOV * 2, 0, aimbotFOV * 2)
        aimbotCircle.Position = UDim2.new(0.5, -aimbotFOV, 0.5, -aimbotFOV)
    end
...
function AimBot.aimAtPlayer(target)
...
function AimBot.toggle(state, screenGui)
    ...
    if state then
        AimBot.createAimbotCircle(screenGui)
        aimbotConnection = RunService.RenderStepped:Connect(function()
            if aimbotEnabled then
                local closest = AimBot.getClosestPlayer()
                if closest then
                    AimBot.aimAtPlayer(closest)
                end
            end
        end)
    else
