...
local Fly = loadModule("https://raw.githubusercontent.com/GermionpiZDA/elysiumPRO/refs/heads/main/elysiumPRO/Tab/Extra/Fly.lua")
...
local function createUIElements()
    ...
    -- ДОБАВЛЯЕМ FLY СЮДА, ВНУТРИ УСЛОВИЯ:
    if Fly then
        local flyToggle = createToggle("Fly Hack", false, function(state)
            Fly.toggle(state)
        end)
        if flyToggle then extraTab.addElement(flyToggle) end
        
        local flySlider = createSlider("Fly Speed", 10, 100, 50, function(value)
            Fly.setSpeed(value)
        end)
        if flySlider then extraTab.addElement(flySlider) end
    end
end
